...
```bash
STACK_NAME=stag-osintbuddy-dev sh ./scripts/deploy.sh
```
...
services:
  db:
    ...
    deploy:
      placement:
        constraints:
          - node.labels.osintbuddy-dev.app-db-data == true
```
...
```bash
docker node update --label-add osintbuddy-dev.app-db-data=true dog.example.com
```

* Then you need to do the same for each stack version you have. For example, for staging you could do:

```bash
docker node update --label-add stag-osintbuddy-dev.app-db-data=true cat.example.com
```
...
* Set these environment variables:
  * `DOMAIN=osintbuddy.dev`
  * `TRAEFIK_TAG=osintbuddy.dev`
  * `STACK_NAME=osintbuddy-dev`
  * `TAG=prod`
...
Frontend: https://osintbuddy.dev

Backend: https://osintbuddy.dev/api/

Automatic Interactive Docs (Swagger UI): https://osintbuddy.dev/docs

Automatic Alternative Docs (ReDoc): https://osintbuddy.dev/redoc

PGAdmin: https://pgadmin.osintbuddy.dev

Flower: https://flower.osintbuddy.dev

### Staging URLs

Staging URLs, from the branch `master`.

Frontend: https://stag.osintbuddy.dev

Backend: https://stag.osintbuddy.dev/api/

Automatic Interactive Docs (Swagger UI): https://stag.osintbuddy.dev/docs

Automatic Alternative Docs (ReDoc): https://stag.osintbuddy.dev/redoc

PGAdmin: https://pgadmin.stag.osintbuddy.dev

Flower: https://flower.stag.osintbuddy.dev
...
Frontend: http://dev.osintbuddy.dev

Backend: http://dev.osintbuddy.dev/api/

Automatic Interactive Docs (Swagger UI): https://dev.osintbuddy.dev/docs

Automatic Alternative Docs (ReDoc): https://dev.osintbuddy.dev/redoc

PGAdmin: http://dev.osintbuddy.dev:5050

Flower: http://dev.osintbuddy.dev:5555

Traefik UI: http://dev.osintbuddy.dev:8090
