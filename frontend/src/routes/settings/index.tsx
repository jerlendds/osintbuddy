...
export default function SettingsPage() {
  ...
  return (
    <>
        ...
        <nav className='flex overflow-x-auto py-4'>
          ...
          >
            {secondaryNavigation.map((item) => (
              <li key={item.name}>
                <a href={item.href} className={classNames('font-display', item.current ? 'text-info-100' : '')}>
                  {item.name}
                </a>
              </li>
            ))}
      ...
      <PageHeader title='' header='Proxy configuration' />
      ...
      <div className='sm:px-2 lg:px-6 my-6 relative mx-auto w-full justify-center'>
        <div>
          ...
          <div className="flex flex-col">
              <h2 className='text-base font-semibold leading-7 text-slate-200'>Your proxies</h2>
          <p className='mt-1 text-sm leading-6 text-slate-400'>Connect proxies to OSINTBuddy one by one or by uploading a txt file of proxies. Each new line in the text file is expected to be one proxy</p>
          </div>
          ...
          <ul role='list' className='mt-6 divide-y divide-light-100 border-t border-dark-300 text-sm leading-6'>
            <li className='flex justify-between gap-x-6 py-6'>
              <div className='font-medium text-slate-400'>https://149.143.12.124:4000</div>
              <button type='button' className='font-semibold text-info-200 hover:text-info-100'>
                Delete
              </button>
            </li>
          </ul>
          ...
          <div className='flex border-t border-dark-300 pt-6'>
            <button type='button' className='text-sm font-semibold leading-6 text-info-200 hover:text-info-100'>
